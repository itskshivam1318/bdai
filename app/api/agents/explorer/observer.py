...
class Observer:
    ...
    def _observation(self, snapshot: str) -> Observation:
        """The one place an Observation is built, and the one place it is redacted.
        ...
        of all three -- redacting typed credentials out of them -- is a property
        of *this* function instead of a rule each return site has to remember.
        Two construction sites that can drift is the shape of the next bug;
        there is one, so "no Observation exists carrying a password" is a claim
        about a dozen lines.

        That is also why the redaction moved in here rather than staying at the
        three call sites it started at: the seam was built for exactly this and
        was still empty only because `redact_url` and `redact_snapshot` lived on
        another branch.
